    ...
    if user_select == server_select:
        ...
        if user_select == 1:
            ...
            if server_select == 1:
                ...
                result = '승리'
    # ajax에서 받는 데이터의 타입을 json으로 저장했기 때문에
    return_data = {
        'user' : user_select,
...
def signup2():
    # 비동기 통신으로 들어오는 데이터의 형태{ user_id : xxxx }
    ...
    print(f"[post] /check_id : DBresult {res_sql}")
    # res_sql의 데이터 타입? DataFrame
    # 길이가 0이면 사용가능, 아니면 사용불가
    if len(res_sql) == 0:
        result = True
    else:
        result = False
...
def signup3():
    ...
    # insert가 정상적으로 작동을 했다면 -> 로그인 페이지('/')로 이동 redirect
    try:
        insert_query = """
            INSERT INTO
            `user_list`
