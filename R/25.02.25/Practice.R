...
midwest
# 미성년자의 인구 비율이 높은 상위 5개의 지역을 출력
# popadult 컬럼의 이름을 adult 변경(rename)
...
midwest %>% 
  ...
  select(county, adult, child, total, child_ratio) %>% 
...
# 그룹화 연산 income의 평균 값 (summerise)
# 변수에 저장
# 막대그래프 시각화해서 표시

df %>% 
  select(gender, income) %>% 
  # 인덱스의 조건식 : 0이 아닌 경우, 9999가 아닌경우, 결측치가 아닌경우, 
  # filter(!ls.na(income)) %>% 
  # filter(income != 0 %>% 
  # filter(income(income = 1~9999))
...
# rbind(male_data, female_data) : 컬럼 수가 맞지 않아 오류
