...
# div 태그중 class가 news_section인 태그를 선택
div_data = soup.find(
...
# li태그 중 첫번째 a태그의 href속성의 값을 리스트로 생성
href_list = list(
    map(
...
for href in href_list:
    try :
        ...
        # h2태그 중 id가 title_area인 태그에서 텍스트 추출
        news_title = news_soup.find(
            'h2',
            attrs = {'id' : 'title_area'}
        ).get_text()
        # div태그 중 id가 newsct_article인 태그에서 텍스트 추출
