...
# 함수 호출
func_1()
# 매개변수가 존재하지 않는 함수에 인자값을 넣으면? -> 에러 발생
# func_1("test")
...
# 절대 경로
  ...
  #     mac -> ~/admin/document/a.csv
  #     web -> https://www.google.com
# 상대 경로
  # 상대적인 주소
  # 환경에 따라서 경로가 변할 수 있다.
  # 현재 위치에서 상위나 하위 폴더로 이동하여 경로 생성
  # ./ : 현재 위치
  # ../ : 상위 폴더로 이동 (부모 폴더)
  #  폴더명/ : 하위 폴더로 이동 (자식 폴더)
...
df
# 절대 경로를 이용하여 데이터 로드
df2 = read.csv("/Users/kimboyun/Desktop/Code/Ubion/R/data/csv_exam.csv")
df2
...
# 파이프 연산자 %>% -> shift + ctrl + m / control + shift + m
...
# 두개의 데이터프레임을 공통적으로 가지고 있는 데이터들의 결합
...
# ratio컬럼 데이터에서 전체 평균 데이터보다 크다면 'large'
# 같다면 'middle'
# 작다면 'small' 데이터를 가지는 새로운 파생변수 생성
