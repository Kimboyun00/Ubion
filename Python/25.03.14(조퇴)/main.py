...
# port : 5000
# 웹서버 접속시 -> localhost:5000

# 누군가가 내 웹 서버에 요청을 보낸다.
# localhost:5000
@app.route('/')
def index():
    # index()함수는 localhost:5000/ 요청이 왔을때 호출
    # 문자열 되돌려줄 때
    # return "<h1>Hello</h1>"
    # html 문서를 되돌려줄 때
...
def get_login():
    ...
    # request 안에 key가 args 되어있는 value를 확인
    print(request.args)
    # 로그인 성공하는 조건?
    ...
    # request는 로그인의 타입이? dict -> {'input_id':'test', 'input_pass':'asdf1234'}
    ...
    # 데이터를 보내는 방법
        # from 태그를 이용
            ...
            # 어떤 방법으로 보낼것인가? (method)
                # get
                    # url 뒤에 ?를 붙이고 key = value형태로 데이터를 전송
...
# 서버가 유저에게 데이터를 보내는 경우
    ...
    # html과 특정 데이터를 묶어서 하나의 html로 만든다.
    # render_template(파일의 이름, html에서 사용할 변수명 = 데이터)
        ...
        # 정체 데이터를 문자열로 변경하여 유저에게 되돌려주는 형태